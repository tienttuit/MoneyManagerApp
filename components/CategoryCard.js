...
import { deleteTransaction } from "../Helper/local-storage-service";
import {
  EXPENSE_DATA,
  INCOME_DATA,
  SAVING_DATA,
  ...
  return (
    ...
    >
      ...
      <View style={styles.title}>
        <Text style={{ fontSize: FONTSIZE.header1, fontWeight: "400" }}>
          {props.title}
        </Text>
      </View>
      ...
      <View style={styles.money}>
        <Text
          style={{
            ...
            fontWeight: "600",
            color: props.type == "-" ? "#FF6363" : "#2FA4FF",
          }}
        >
          {props.moneyValue ? props.type : ""}
          {formatMoney(props.moneyValue)} {props.moneyValue ? "VND" : ""}{" "}
        </Text>
      </View>
    </TouchableOpacity>
  );
...
const onAlert = (id) => {
  Alert.alert(
    ...
    [{ text: "Hủy" }, { text: "Chắc chắn", onPress: DeleteCategory(id) }]
  );
};
...
const DeleteCategory = (id) => {
    ...
    for (let i = 0; i < object.length; i++) {
      if (object[i].id == x) {
        if (object[i].canDelete == false) {
          Alert.alert(
            "Thông báo",
            "Bạn không thể xóa nội dung mặc định của nhà phát triển"
          );
          return -1;
        }
        return i;
  ...
  switch (id.slice(0, 1)) {
    case "e":
      if (checkId(EXPENSE_DATA, id) == -1) break;
      EXPENSE_DATA.splice(checkId(EXPENSE_DATA, id), 1);
      break;

    case "s":
      if (checkId(SAVING_DATA, id) == -1) break;
      SAVING_DATA.splice(checkId(SAVING_DATA, id), 1);
      break;

    case "i":
      if (checkId(INCOME_DATA, id) == -1) break;
