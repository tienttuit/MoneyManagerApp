...
const SavingGoalCard = (props) => {
  ...
  useEffect(() => {
    if (progress_perc >= 1) updateSavingGoalStatus(props.item.goalID);
  }, [progress_perc]);
  return (
    <TouchableOpacity style={styles.container} onPress={() => props.onPress()}>
      <View style={{ padding: 5 }}>
        <Text style={{ fontSize: FONTSIZE.header1, fontWeight: "600" }}>
          {props.item.goalName}
        </Text>
      </View>
      <View style={{ padding: 5, paddingVertical: 10, flexDirection: "row" }}>
        ...
        <Text style={{ fontSize: FONTSIZE.header1, fontWeight: "600" }}>
          {" "}
          {formatMoney(props.item.savingValue)}
        </Text>
      </View>
      <View
        style={{
      ...
      >
        <Progress.Bar
          progress={progress_perc > 1 ? 1 : progress_perc}
          width={280}
          unfilledColor={"rgb(248,248,248)"}
          height={15}
          color={"rgb(61,186,171)"}
          borderColor={"black"}
        />
      ...
      <View style={{ padding: 5, flexDirection: "row", alignItems: "center" }}>
        <MaterialCommunityIcons
          ...
          color="black"
        />
        <Text style={{ fontSize: FONTSIZE.body, fontWeight: "500" }}>
          {" "}
          Tiến độ: {formatMoney(props.item.currentMoney)} /{" "}
          {formatMoney(props.item.savingValue)} VND
        </Text>
      </View>
      <View style={{ padding: 5, flexDirection: "row", alignItems: "center" }}>
        ...
        <Text style={{ fontSize: FONTSIZE.body, fontWeight: "500" }}>
          {" "}
          Ngày kết thúc: {new Date(props.item.date).getDate()}/
